...
**問題1**：for-inループを使って、次のリストの点数をすべて表示し、最後に合計点を表示するプログラムを書いてください。

```dart
List<int> scores = [80, 65, 92, 74];
```
...
void main() {
  ...
  print('合計: $total点');  // 合計: 311点
}
```

</details>

**問題2**：次のリストから、70点以上の点数だけを数えて「合格者は◯人です」と表示するプログラムを書いてください。

```dart
List<int> scores = [55, 82, 70, 91, 68, 77];
```
...
void main() {
  ...
  print('合格者は$passCount人です');  // 合格者は4人です
}
```

ループと条件分岐を組み合わせて「条件を満たす要素を数える」のは、実際のプログラムでも頻出のパターンです。
