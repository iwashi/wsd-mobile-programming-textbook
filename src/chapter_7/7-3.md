...
**問題1**：次のプログラムを実行した後の `members` の中身を予想してください。

```dart
void main() {
  List<String> members = ['田中', '佐藤'];
...
順に追うと：
1. `add('鈴木')` → `[田中, 佐藤, 鈴木]`
2. `insert(0, '高橋')` → 先頭に追加されて `[高橋, 田中, 佐藤, 鈴木]`
3. `remove('佐藤')` → `[高橋, 田中, 鈴木]`

</details>

**問題2**：空のリストから始めて、「牛乳」「パン」「卵」を追加し、その後「パン」を削除して、最終的なリストと要素数を表示する買い物リストプログラムを書いてください。
...
void main() {
  ...
  print('買い物リスト: $shoppingList');       // 買い物リスト: [牛乳, 卵]
  print('アイテム数: ${shoppingList.length}'); // アイテム数: 2
}
```
