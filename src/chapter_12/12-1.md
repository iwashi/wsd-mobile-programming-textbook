...
第11章の11-2で「オブジェクトを作るときに値を渡す仕組み」としてコンストラクタに少しだけ触れました。この章では、その使い方を詳しく学びます。
...
```dart
class Rectangle {
  // 名前付きコンストラクタから代入できるよう、初期値を設定しておく
  double width = 0.0;
  double height = 0.0;
...
次の例には、新しい書き方「**イニシャライザリスト**」が登場します。コンストラクタの `()` の後に `: フィールド = 値, フィールド = 値` と書くと、コンストラクタ本体（`{}` の中）が実行される**前に**フィールドへ値を設定できます。「引数から受け取らないフィールドに、決まった初期値を入れたい」ときによく使う書き方です。
