...
void main() {
  ...
  void editWord(String word, String newMeaning) {
    ...
    if (dictionary.containsKey(lowerWord)) {
      // containsKeyで確認済みなので「!」で安全に取り出せる（8-2参照）
...
**問題1**：この辞書プログラムに「単語数を表示する」機能を追加するとしたら、どう書けばよいでしょうか。マップの要素数を調べる方法を思い出して考えてください。

<details>
<summary>解答例</summary>

```dart
print('登録されている単語数: ${dictionary.length}語');
```

マップもリストと同じように `length` で要素数を取得できます。

</details>

**問題2**：日本語の都道府県名をキー、県庁所在地を値とするミニ辞書（3件以上）を作り、調べたい県名が辞書に「あれば県庁所在地を表示、なければ『登録されていません』と表示」するプログラムを書いてください。
...
void main() {
  ...
  if (capitals.containsKey(query)) {
    print('$queryの県庁所在地は${capitals[query]}です');
  } else {
    print('$queryは登録されていません');
  }
}
```

`query` を `'東京都'` などに変えて、「登録されていません」が表示されることも確認してみましょう。
