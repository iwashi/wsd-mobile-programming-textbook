...
2-3で学んだとおり、`$`記号を使って文字列の中に変数の値を埋め込むことができます。文字列を変数として扱う場合も同じです。
...
// ${式} で計算結果も埋め込み可能
String detail = "私は$nameです。5年後は${age + 5}歳になります。";
print(detail); // 出力：私は佐藤です。5年後は30歳になります。
